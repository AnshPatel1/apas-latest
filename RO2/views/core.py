...
from Faculty.FacultyFOET.models import FOETAssistantProfOnContractAppraisalFile, FOETAssistantProfAppraisalFile, \
    FOETAssociateProfAppraisalFile, FOETProfAppraisalFile
...
class RO2MainView:
    ...
    def dashboard(request):
        if request.method == 'GET':
            if request.user.is_ro2:
                context = {}
                context['pagename'] = 'ro2-dashboard'
                # try:
                ...
                # context['files'] = StaffAppraisalFile.objects.filter(file_level='RO2')
                return render(request, 'html/r2/ro2-dashboard.html', context)
            else:
                context = {
    ...
    #         if request.user.is_ro2:
    #             context = {}
    #             context['pagename'] = 'ro2-profile'
    #             return render(request, 'html/r2/ro2-profile.html', context)
    #         else:
    #             context = {
    #                 'error_code': "Unauthorized Error",
    #                 "error_message": "You are not authorized to view this page."
        ...
        if request.method == 'GET':
            if request.user.is_ro2:
                ...
                for i in context['pending_files']:
                    if i.designation not in context['designations']:
                        context['designations'].append(i.designation)
                return render(request, 'html/r2/r2-review-main.html', context)
            else:
                context = {
                    'error_code': "Unauthorized Error",
                    "error_message": "You are not authorized to view this page."
    ...
    def review_faculty(request):
        if request.user.is_ro1:
            context = {}
            context['pagename'] = 'ro2-review-faculty'
            ...
            science_files = []
            context['cycles'] = {
                'foet': RO2MainView.get_cycle('foet'),
                'sls': RO2MainView.get_cycle('sls'),
                'som': RO2MainView.get_cycle('som'),
                'math': RO2MainView.get_cycle('math'),
                'science': RO2MainView.get_cycle('science'),
            }
            ...
            return render(request, 'html/r2/r2-review-faculty.html', context)
        else:
            context = {
                'error_code': "Unauthorized Error",
                "error_message": "You are not authorized to view this page."
