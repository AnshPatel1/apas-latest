...
class CalculationEngine:
    ...
    def __init__(self,
                 file: FOETAssistantProfOnContractAppraisalFile | FOETAssistantProfAppraisalFile | FOETAssociateProfAppraisalFile | FOETProfAppraisalFile):
        ...
        for award in self.file.recognition_awards.all():
            if award.marks.ro1_agreed:
                if award.award_type == 'international':
                    total += self.file.configuration.section_2e_international_level
                    award.marks.ro1 = self.file.configuration.section_2e_international_level
                elif award.award_type == 'national':
                    total += self.file.configuration.section_2e_national_level
                    award.marks.ro1 = self.file.configuration.section_2e_national_level
                elif award.award_type == 'state':
                    total += self.file.configuration.section_2e_state_level
                    award.marks.ro1 = self.file.configuration.section_2e_state_level
                elif award.award_type == 'university' or award.award_type == 'regional':
                    total += self.file.configuration.section_2e_regional_university_level
                    award.marks.ro1 = self.file.configuration.section_2e_regional_university_level
            else:
                award.marks.ro1 = 0
            award.marks.save()
        if total > self.file.configuration.section_2e_recognition_awards_received_max_marks:
            total = self.file.configuration.section_2e_recognition_awards_received_max_marks
        return round(total, 2)
...
class CalculationEngineR2:
    ...
    def calculateAwards(self):
        ...
        for award in self.file.recognition_awards.all():
            if award.marks.ro2_agreed:
                if award.award_type == 'international':
                    total += self.file.configuration.section_2e_international_level
                    award.marks.ro2 = self.file.configuration.section_2e_international_level
                elif award.award_type == 'national':
                    total += self.file.configuration.section_2e_national_level
                    award.marks.ro2 = self.file.configuration.section_2e_national_level
                elif award.award_type == 'state':
                    total += self.file.configuration.section_2e_state_level
                    award.marks.ro2 = self.file.configuration.section_2e_state_level
                elif award.award_type == 'university' or award.award_type == 'regional':
                    total += self.file.configuration.section_2e_regional_university_level
                    award.marks.ro2 = self.file.configuration.section_2e_regional_university_level
            else:
                award.marks.ro2 = 0
            award.marks.save()
        if total > self.file.configuration.section_2e_recognition_awards_received_max_marks:
    ...
    def calculateMentoring(self):
        total = 0
        if self.file.being_mentor_total is None:
            self.file.being_mentor_total.ro2 = MarkField()
        if self.file.being_mentor_available:
