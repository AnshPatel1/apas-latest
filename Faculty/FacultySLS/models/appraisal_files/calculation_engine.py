from . import FOLSAssistantProfOnContractAppraisalFile, FOLSAssistantProfAppraisalFile, FOLSAssociateProfAppraisalFile, \
...
class CalculationEngine:
    ...
    def __init__(self,
                 file: FOLSAssistantProfOnContractAppraisalFile | FOLSAssistantProfAppraisalFile | FOLSAssociateProfAppraisalFile | FOLSProfAppraisalFile):
        ...
        for paper in self.file.publications_in_scopus_wos.all():
            if paper.marks.ro1_agreed:
                if paper.is_main_author:
                    ...
                    elif paper.category == 'ugc' or paper.category == 'ugc':
    ...
    def calculateAttendingConferences(self):
        ...
        for conference in self.file.attending_conferences.all():
            if conference.marks.ro1_agreed:
                total += self.file.configuration.section_4a_attending_conferences_marks_per_conference
                conference.marks.ro1 = self.file.configuration.section_4a_attending_conferences_marks_per_conference
            else:
                conference.marks.ro1 = 0

            conference.marks.save()
        return min(total, self.file.configuration.section_4a_attending_conferences_max_marks)
...
class CalculationEngineR2:
    ...
    def calculateResearchMaster(self):
        total = 0
        total += self.calculatePapers()
        # total += self.calculatePapersSciUGC()
        total += self.calculateConferences()
        total += self.calculateEPublications()
        return total
    ...
    def calculateBachelorsDissertation(self):
        ...
        count = 0
        for dissertation in self.file.bachelors_dissertation.all():
            if dissertation.marks.ro2_agreed and dissertation.is_awarded and count <= self.file.configuration.section_2c_i_max_dissertation:
                count += 1
                total += self.file.configuration.section_2c_i_marks_per_dissertation_awarded
    ...
    def calculateAcademiaCollaboration(self):
        if self.file.academia_collaboration_total is None:
            self.file.academia_collaboration_total = MarkField()
        self.file.academia_collaboration.mou_marks.ro2 = self.file.academia_collaboration.mou_marks.ro1
        self.file.academia_collaboration.contribution_marks.ro2 = self.file.academia_collaboration.contribution_marks.ro1
        self.file.academia_collaboration_total.ro2 = self.file.academia_collaboration.mou_marks.ro1 + self.file.academia_collaboration.contribution_marks.ro1
        self.file.academia_collaboration_total.save()
        self.file.academia_collaboration.mou_marks.save()
        self.file.academia_collaboration.contribution_marks.save()
        self.file.save()
    ...
    def calculateInternationalAdmission(self):
        ...
        self.file.international_admission.save()
        return self.file.international_admission.ro2
    ...
    def calculateAttendingConferences(self):
        ...
        for conference in self.file.attending_conferences.all():
            if conference.marks.ro2_agreed:
                total += self.file.configuration.section_4a_attending_conferences_marks_per_conference
                conference.marks.ro2 = self.file.configuration.section_4a_attending_conferences_marks_per_conference
            else:
