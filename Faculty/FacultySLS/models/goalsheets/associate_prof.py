...
class FOLSGoalSheetAssociateProf(models.Model):
    ...
    section_3_subtotal = models.IntegerField(default=25)
    ...
    def __str__(self):
        if self.is_active:
            return f"FoLS Associate Professor Goalsheet for {self.year} (Active)"
        else:
            return f"FoLS Associate Professor Goalsheet Goalsheet for {self.year}"

    class Meta:
        verbose_name = "Goalsheet Configuration for FoLS Associate Professor"
        verbose_name_plural = "Goalsheet Configurations for FoLS Associate Professor"
