...
class FOLSGoalSheetAssistantProfOnContract(models.Model):
    """This is the FoLS Goalsheet model. It contains the fields for the FoLS Goalsheet form."""
    # Configurations
    year = models.IntegerField()
    is_active = models.BooleanField(default=False)

    grand_total = models.IntegerField(default=384)

    # ||||||||||||||||||||||   PART A   ||||||||||||||||||||||
    # TOTAL FOR PART A (SECTION 1 TO 4) SHOULD BE 307
    part_a_total = models.IntegerField(default=344)
    ...
    section_2_subtotal = models.IntegerField(default=182)
    ...
    section_3_subtotal = models.IntegerField(default=20)
    ...
    def __str__(self):
        if self.is_active:
            return f"FoLS Asst. Prof. On Contract Goalsheet for {self.year} (Active)"
        else:
            return f"FoLS Asst. Prof. On Contract Goalsheet for {self.year}"
