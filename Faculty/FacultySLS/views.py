...
from .models import *
from BulkUpload.BulkUploadFoLS.models import *
from MasterConfiguration.models import SLSFacultyAppraisalCycleConfiguration, SLSFacultyAppraisalCycleInclusion, CurrentDate, ShowResult
...
class FacultyViewSet:
    ...
    def dashboard(request):
        ...
        if file is not None:
            context['file'] = file
        return render(request, "html/faculty/fols/dashboard.html", context)
    ...
    def verify_data(request):
        ...
        except ViewInternationalAdmission.DoesNotExist:
            return HttpResponse("No Additional Marks Found")
        except ViewInternationalAdmission.MultipleObjectsReturned:
            return HttpResponse("Multiple International Admission Marks Found. Please contact admin to get this fixed.")
        ...
        if request.method == 'POST' and not file.has_verified_data:
            ...
            if file.has_verified_data and request.method == 'POST':
                return render(request, "html/faculty/fols/verify-data.html", context)
    ...
    def teaching_entry(request):
        ...
        return render(request, "html/faculty/fols/teaching.html", context)
    ...
    def phd_guidance_entry(request):
        ...
        return render(request, "html/faculty/fols/phd-guidance-entry.html", context)
    ...
    def award_entry(request):
        ...
        if file is not None:
            context['file'] = file
        return render(request, "html/faculty/fols/award-entry.html", context)
    ...
    def consultancy_entry(request):
        ...
        return render(request, "html/faculty/fols/consultancy-entry.html", context)
    ...
    def academia_collab_entry(request):
        ...
        if file is not None:
            context['file'] = file
        return render(request, "html/faculty/fols/academia-collab-entry.html", context)

    @staticmethod
    @login_required(login_url='/login/')
    def international_admission_entry(request):
        if not FacultyHelperFunctions.check_authorized_user(request):
            context = {
                'error_code': "Unauthorized Error",
                "error_message": "You are not authorized to view this page."
            ...
            return render(request, "html/error_pages/pages-error.html", context)
        context = {'user': request.user, 'page_name': 'international-admission-entry'}
        file = FacultyHelperFunctions.get_appraisal_file(request)
        context['cycle'] = FacultyHelperFunctions.get_cycle()
        if file is not None:
            context['file'] = file
        return render(request, "html/faculty/fols/international-admission-entry.html", context)
    ...
    def arranging_conference_entry(request):
        ...
        return render(request, "html/faculty/fols/arranging-conference.html", context)
    ...
    def mentorship_entry(request):
        ...
        if request.method == 'POST':
            ...
            if available:
                ...
                file.being_mentor.all().delete()
        return render(request, "html/faculty/fols/mentorship-entry.html", context)
    ...
    def attending_conference_entry(request):
        ...
        if request.method == 'POST':
            ...
            if available:
                ...
                file.attending_conferences.all().delete()
        return render(request, "html/faculty/fols/attending-conferences-entry.html", context)
    ...
    def community_development_entry(request):
        ...
        if request.method == 'POST':
            ...
            if available:
                ...
                file.community_development.all().delete()
        return render(request, "html/faculty/fols/community-development-entry.html", context)
    ...
    def extra_curricular_entry(request):
        ...
        if file is not None:
            context['file'] = file
        return render(request, "html/faculty/fols/extra-curricular-entry.html", context)
    ...
    def additional_entry(request):
        ...
        return render(request, "html/faculty/fols/additional-entry.html", context)
    ...
    def submit(request):
        ...
        if request.method == 'POST':
            file.file_level = 'RO1'
            file.save()
        return render(request, "html/faculty/fols/submit.html", context)
    ...
    def summary(request):
        ...
        context['external_guidance'] = file.phd_guidance.filter(category='external')
        context['internal_guidance'] = file.phd_guidance.filter(category='internal')
        return render(request, "html/faculty/fols/download.html", context)
...
class FacultyHelperFunctions:
    ...
    def check_authorized_user(request):
        ...
        if request.user.is_authenticated and request.user.is_active and request.user.roles == 'fac' and SLSFacultyAppraisalCycleInclusion.check_inclusion(request.user):
            return True
        else:
            return False

    @staticmethod
    def get_appraisal_file(request) -> FOLSAssistantProfOnContractAppraisalFile | FOLSAssistantProfAppraisalFile | FOLSAssociateProfAppraisalFile | FOLSProfAppraisalFile:
        try:
            file = None
            if request.user.designation_abbreviation == 'assistant_prof_on_contract':
                try:
                    file = FOLSAssistantProfOnContractAppraisalFile.objects.get(user=request.user)
                except FOLSAssistantProfOnContractAppraisalFile.DoesNotExist:
                    file = FOLSAssistantProfOnContractAppraisalFile()
                    file.user = request.user
                    file.configuration = FOLSGoalSheetAssistantProfOnContract.objects.filter(is_active=True).first()
                    file.save()
            elif request.user.designation_abbreviation == 'assistant_prof':
                try:
                    file = FOLSAssistantProfAppraisalFile.objects.get(user=request.user)
                except FOLSAssistantProfAppraisalFile.DoesNotExist:
                    file = FOLSAssistantProfAppraisalFile()
                    file.user = request.user
                    file.configuration = FOLSGoalSheetAssistantProf.objects.filter(is_active=True).first()
                    file.save()
            elif request.user.designation_abbreviation == 'associate_prof':
                try:
                    file = FOLSAssociateProfAppraisalFile.objects.get(user=request.user)
                except FOLSAssociateProfAppraisalFile.DoesNotExist:
                    print("hi")
                    file = FOLSAssociateProfAppraisalFile()
                    file.user = request.user
                    file.configuration = FOLSGoalSheetAssociateProf.objects.filter(is_active=True).first()
                    file.save()
            elif request.user.designation_abbreviation == 'prof':
                try:
                    file = FOLSProfAppraisalFile.objects.get(user=request.user)
                except FOLSProfAppraisalFile.DoesNotExist:
                    file = FOLSProfAppraisalFile()
                    file.user = request.user
                    file.configuration = FOLSGoalSheetProf.objects.filter(is_active=True).first()
                    file.save()
            return file
    ...
    def get_cycle():
        ...
        except SLSFacultyAppraisalCycleConfiguration.DoesNotExist:
