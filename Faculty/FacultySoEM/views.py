...
from .models import *
from BulkUpload.BulkUploadFoEM.models import *
from MasterConfiguration.models import SOEMFacultyAppraisalCycleConfiguration, SPMFacultyAppraisalCycleInclusion, CurrentDate, ShowResult
...
class FacultyViewSet:
    ...
    def dashboard(request):
        ...
        if file is not None:
            context['file'] = file
        return render(request, "html/faculty/foem/dashboard.html", context)
    ...
    def verify_data(request):
        ...
        try:
            ...
            return HttpResponse("Multiple International Admission Marks Found. Please contact admin to get this fixed.")
        ...
        if request.method == 'POST' and not file.has_verified_data:
            ...
            file.teaching_load.semester_third = context['teaching_load'].load_tri
            ...
            for book in context['books']:
                ...
                elif book.category.lower() == 'literary':
                    publication.category = 'literary'
            ...
            if file.pgp_chair_marks is None:
                file.pgp_chair_marks = MarkField()
            file.pgp_chair_marks.ro1 = context['additional'].pgp_chair
            file.pgp_chair_marks.ro1_remarks = 'From: Director\'s Office'
            file.pgp_chair_marks.ro1_agreed = True
            file.pgp_chair_marks.save()

            if file.e_mba_chair_marks is None:
                file.e_mba_chair_marks = MarkField()
            file.e_mba_chair_marks.ro1 = context['additional'].e_mba_chair
            file.e_mba_chair_marks.ro1_remarks = 'From: Director\'s Office'
            file.e_mba_chair_marks.ro1_agreed = True
            file.e_mba_chair_marks.save()

            file.save()
        else:
            if file.has_verified_data and request.method == 'POST':
                return render(request, "html/faculty/foem/verify-data.html", context)
    ...
    def teaching_entry(request):
        ...
        file = FacultyHelperFunctions.get_appraisal_file(request)
        context['cycle'] = FacultyHelperFunctions.get_cycle()
        context['books'] = file.textbooks.all().union(file.research_books.all().union(file.literary_books.all()))
        if file is not None:
            context['file'] = file
        if request.method == 'POST':
        ...
        return render(request, "html/faculty/foem/teaching.html", context)
    ...
    def phd_guidance_entry(request):
        ...
        return render(request, "html/faculty/foem/phd-guidance-entry.html", context)
    ...
    def award_entry(request):
        ...
        if file is not None:
            context['file'] = file
        return render(request, "html/faculty/foem/award-entry.html", context)
    ...
    def consultancy_entry(request):
        ...
        if request.method == 'POST':
            ...
            file.validator.save()
        if request.user.designation_abbreviation == 'assistant_prof_on_contract' and len(file.providing_consultancy.all()) == 0:
            # if True:
            if request.method == 'POST':
        ...
        return render(request, "html/faculty/foem/consultancy-entry.html", context)
    ...
    def academia_collab_entry(request):
        ...
        if file is not None:
            context['file'] = file
        return render(request, "html/faculty/foem/academia-collab-entry.html", context)
    ...
    def arranging_conference_entry(request):
        ...
        return render(request, "html/faculty/foem/arranging-conference.html", context)
    ...
    def mentorship_entry(request):
        ...
        if request.method == 'POST':
            ...
            if available:
                ...
                file.being_mentor.all().delete()
        return render(request, "html/faculty/foem/mentorship-entry.html", context)
    ...
    def attending_conference_entry(request):
        ...
        if request.method == 'POST':
            ...
            if available:
                ...
                file.attending_conferences.all().delete()
        return render(request, "html/faculty/foem/attending-conferences-entry.html", context)
    ...
    def community_development_entry(request):
        ...
        if request.method == 'POST':
            ...
            if available:
                ...
                file.community_development.all().delete()
        return render(request, "html/faculty/foem/community-development-entry.html", context)
    ...
    def extra_curricular_entry(request):
        ...
        if file is not None:
            context['file'] = file
        return render(request, "html/faculty/foem/extra-curricular-entry.html", context)
    ...
    def additional_entry(request):
        ...
        return render(request, "html/faculty/foem/additional-entry.html", context)
    ...
    def submit(request):
        ...
        if request.method == 'POST':
            file.file_level = 'RO1'
            file.save()
        return render(request, "html/faculty/foem/submit.html", context)
    ...
    def summary(request):
        ...
        context['external_guidance'] = file.phd_guidance.filter(category='external')
        context['internal_guidance'] = file.phd_guidance.filter(category='internal')
        return render(request, "html/faculty/foem/download.html", context)
    ...
    def result(request):
        ...
        return render(request, "html/faculty/foem/result.html", context)
...
class FacultyHelperFunctions:
    ...
    def check_authorized_user(request):
        ...
        if request.user.is_authenticated and request.user.is_active and request.user.roles == 'fac' and SPMFacultyAppraisalCycleInclusion.check_inclusion(request.user):
            return True
        else:
            return False

    @staticmethod
    def get_appraisal_file(request) -> FOEMAssistantProfOnContractAppraisalFile | FOEMAssistantProfAppraisalFile | FOEMAssociateProfAppraisalFile | FOEMProfAppraisalFile:
        try:
            file = None
            if request.user.designation_abbreviation == 'assistant_prof_on_contract':
                try:
                    file = FOEMAssistantProfOnContractAppraisalFile.objects.get(user=request.user)
                except FOEMAssistantProfOnContractAppraisalFile.DoesNotExist:
                    file = FOEMAssistantProfOnContractAppraisalFile()
                    file.user = request.user
                    file.configuration = FOEMGoalSheetAssistantProfOnContract.objects.filter(is_active=True).first()
                    file.save()
            elif request.user.designation_abbreviation == 'assistant_prof':
                try:
                    file = FOEMAssistantProfAppraisalFile.objects.get(user=request.user)
                except FOEMAssistantProfAppraisalFile.DoesNotExist:
                    file = FOEMAssistantProfAppraisalFile()
                    file.user = request.user
                    file.configuration = FOEMGoalSheetAssistantProf.objects.filter(is_active=True).first()
                    file.save()
            elif request.user.designation_abbreviation == 'associate_prof':
                try:
                    file = FOEMAssociateProfAppraisalFile.objects.get(user=request.user)
                except FOEMAssociateProfAppraisalFile.DoesNotExist:
                    print("hi")
                    file = FOEMAssociateProfAppraisalFile()
                    file.user = request.user
                    file.configuration = FOEMGoalSheetAssociateProf.objects.filter(is_active=True).first()
                    file.save()
            elif request.user.designation_abbreviation == 'prof':
                try:
                    file = FOEMProfAppraisalFile.objects.get(user=request.user)
                except FOEMProfAppraisalFile.DoesNotExist:
                    file = FOEMProfAppraisalFile()
                    file.user = request.user
                    file.configuration = FOEMGoalSheetProf.objects.filter(is_active=True).first()
                    file.save()
            return file
