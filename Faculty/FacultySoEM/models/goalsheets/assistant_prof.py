...
class FOEMGoalSheetAssistantProf(models.Model):
    """This is the FoEM Goalsheet model. It contains the fields for the FoLS Goalsheet form."""
    # Configurations
    year = models.IntegerField()
    is_active = models.BooleanField(default=False)

    grand_total = models.IntegerField(default=407)

    # ||||||||||||||||||||||   PART A   ||||||||||||||||||||||
    # TOTAL FOR PART A (SECTION 1 TO 4) SHOULD BE 307
    part_a_total = models.IntegerField(default=367)
    ...
    # ------- Section 4.A: Attending Conferences/ Seminars -------
    ...
    # TODO: RENAME THIS TO section_4a_attending_conferences_max_marks (as it is not arranging conferences)
    ...
    def __str__(self):
        if self.is_active:
            return f"FoEM Assistant Professor Goalsheet for {self.year} (Active)"
        else:
            return f"FoEM Assistant Professor Goalsheet for {self.year}"

    class Meta:
        verbose_name = "Goalsheet Configuration for FoEM Assistant Professor"
        verbose_name_plural = "Goalsheet Configurations for FoEM Assistant Professor"
