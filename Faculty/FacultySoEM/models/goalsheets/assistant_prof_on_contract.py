...
class FOEMGoalSheetAssistantProfOnContract(models.Model):
    """This is the FoEM Goalsheet model. It contains the fields for the FoEM Goalsheet form."""
    # Configurations
    year = models.IntegerField()
    is_active = models.BooleanField(default=False)

    grand_total = models.IntegerField(default=339)

    # ||||||||||||||||||||||   PART A   ||||||||||||||||||||||
    # TOTAL FOR PART A (SECTION 1 TO 4) SHOULD BE 307
    part_a_total = models.IntegerField(default=299)
    ...
    section_2_subtotal = models.IntegerField(default=137)
    ...
    def __str__(self):
        if self.is_active:
            return f"FoEM Assistant Professor On Contract Goalsheet for {self.year} (Active)"
        else:
            return f"FoEM Assistant Professor On Contract Goalsheet for {self.year}"

    class Meta:
        verbose_name = "Goalsheet Configuration for FoEM Assistant Professor On Contract"
        verbose_name_plural = "Goalsheet Configurations for FoEM Assistant Professor On Contract"
