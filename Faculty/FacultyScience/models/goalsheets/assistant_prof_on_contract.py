...
class ScienceGoalSheetAssistantProfOnContract(models.Model):
    """This is the Maths Goalsheet model. It contains the fields for the Maths Goalsheet form."""
    # Configurations
    year = models.IntegerField()
    is_active = models.BooleanField(default=False)

    grand_total = models.IntegerField(default=351)

    # ||||||||||||||||||||||   PART A   ||||||||||||||||||||||
    # TOTAL FOR PART A (SECTION 1 TO 4) SHOULD BE 307
    part_a_total = models.IntegerField(default=311)
    ...
    section_2_subtotal = models.IntegerField(default=124)
    ...
    def __str__(self):
        if self.is_active:
            return f"Science Asst. Prof. On Contract Goalsheet for {self.year} (Active)"
        else:
            return f"Science Asst. Prof. On Contract Goalsheet for {self.year}"

    class Meta:
        verbose_name = "Goalsheet Configuration for Science Assistant Professor On Contract"
        verbose_name_plural = "Goalsheet Configurations for Science Assistant Professor On Contract"
